...
typedef struct BPlusTree
{
    ...
    struct BPlusTree* children[2 * T + 1];
    struct BPlusTree* left;
    struct BPlusTree* right;
}BPlusTree;

BPlusTree* CreateTree(void)
{
...
void DestroyTree(BPlusTree* tree)
{
    if (!tree)
    {
        return;
...
BPlusTree* FindLeaf(BPlusTree* tree, int key)
{
    if (!tree)
    {
        return NULL;
...
int Find(BPlusTree* tree, int key)
{
    BPlusTree* node = FindLeaf(tree, key);
...
int FindMinimum(BPlusTree* tree)
{
    for (int i = 0; i < 2 * T; i++)
    {
        if (tree->children[i])
...
void FixKeys(BPlusTree* node, int key)
{
    BPlusTree* parent = node->parent;
    int i = 0;
...
void Split(BPlusTree** tree, BPlusTree* node)
{
    int key = 0;
    int pos = 0;

    BPlusTree* new_tree = CreateTree();
    new_tree->is_leaf = 0;
...
void Add(BPlusTree** tree, int key)
{
    BPlusTree* leaf = FindLeaf((*tree), key);
...
void DeleteInNode(BPlusTree* tree, BPlusTree* node, int key);
void Merge(BPlusTree* tree, BPlusTree* node, int key)
{
    BPlusTree* parent = node->parent;
    BPlusTree* right = node->right;
    BPlusTree* left = node->left;
    BPlusTree* tmp = NULL;
